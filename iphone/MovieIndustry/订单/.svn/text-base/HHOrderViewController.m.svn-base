...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    if (arrGoods.count>0) {
        ...
        tbFooterView.goodsPriceTotalLabel.text = [NSString stringWithFormat:@"合计：￥%.2f ",(float)amount];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    comfirmOrderVC.goodsModel = dataModel;
...
#warning 退款按钮干掉了
            ...
            {
                if([orderStatus intValue]==6)//商家取消了订单
                {
                   
                    [view.leftBtn setTitle:@"不同意取消" forState:UIControlStateNormal];
                    [view.leftBtn addTarget:self action:@selector(actionBuyerDisagreeCancelOrder:) forControlEvents:UIControlEventTouchUpInside];
                    
                    [view.rightBtn setTitle:@"同意取消" forState:UIControlStateNormal];
                    [view.rightBtn addTarget:self action:@selector(actionBuyerAgreeCancelOrder:) forControlEvents:UIControlEventTouchUpInside];
                }
                else
                {
...
- (void)gotoCommentView:(UIButton *)sender
{
    ...
    commentVC.orderModel = dataModel;
...
- (void)warnSenderMineOrder:(UIButton *)button
{
    ...
    UIAlertAction *confirmAction  = [UIAlertAction actionWithTitle:@"确认" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if(button.tag<20000)
        {
           a = (int)button.tag -10000;
        }else
        {
            a =(int)button.tag -20000;
        }
        NSMutableDictionary * dict = [NSMutableDictionary dictionary];
        dict[@"user_id"] = APP_DELEGATE.user_id;
        dict[@"status"] = @"0";
 ...
 *  @param button 按钮对象
 */
-(void)actionBuyerDisagreeCancelOrder:(UIButton*)button
{
    int a = (int)button.tag -10000;
    NSMutableDictionary * dict = [NSMutableDictionary dictionary];
    dict[@"user_id"] = APP_DELEGATE.user_id;
    dict[@"status"] = @"3";
    OrderDataModel * dataModel = self.dataArray[a];
    dict[@"order_id"] = dataModel.order_id;
    MBProgressHUD *hud  = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    [HttpRequestServers requestBaseUrl:TIShopOrder_OrderStatus withParams:dict withRequestFinishBlock:^(id result) {
        NSDictionary *dict = result;
        HHNSLog(@"%@",dict);
        @try {
            [hud hide:YES];
 ...
 *  @param button 按钮对象
 */
-(void)actionBuyerAgreeCancelOrder:(UIButton *)button
{
