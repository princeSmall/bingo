    ...
    [bottomView  createMyTableViewAndEndBlock:^(NSString *type) {
        if ([type isEqualToString:@"0"]) {
            ...
            if (!self.payDict) {
                ...
                [HttpRequestServers sendAlipayWithOrderSn:self.orderPayDic[@"order_id"] orderName:@"咔么支付测试" orderDescription:@"1分钱测试" orderPrice:[NSString stringWithFormat:@"%f",0.01] andScallback:^(id obj)
                 {
            ...
            else
            {
                ...
                NSDictionary *dict = [NSDictionary  dictionaryWithDictionary:  self.orderPayDic];
                NSLog(@"dict : %@",dict);
                if([self judgeCreateOrder:mutDic]==NO)
                {
                    [DeliveryUtility showMessage:@"订单内商品送货方式不一致" target:self];
                    
                }
                else
                {
                mutDic[@"order_score"]=@"0";
                [HttpRequestServers requestBaseUrl:TIOrder_AddOrder withParams:mutDic withRequestFinishBlock:^(id result) {
                    ...
                    @try {
                        ...
                        [[NSOperationQueue mainQueue]addOperationWithBlock:^{
                            [HttpRequestServers sendAlipayWithOrderSn:self.orderId orderName:@"咔么测试" orderDescription:@"1分钱测试" orderPrice:[NSString stringWithFormat:@"%.2f",0.01] andScallback:^(id obj) {
                                [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
                                NSDictionary *aliDict = obj;
                                HHNSLog(@"支付回调参数 aliDict %@",aliDict);
                                ...
                                if ([aliDict[@"resultStatus"] isEqualToString:@"6001"]) {
                                    ...
                                    APP_DELEGATE.tbbC.selectedViewController = APP_DELEGATE.tbbC.viewControllers[1];
                    ...
                    else
                    {
                        ...
                        [DeliveryUtility showMessage:@"商品库存不足" target:self];
                    }
 ...
 *  判断订单内的商品是是否送货方式一致
 *
 *  @param order_dict 订单词典
 */
-(BOOL)judgeCreateOrder:(NSDictionary *)order_dict
{
    NSMutableArray *goodsArray = [NSMutableArray array];
    goodsArray  = order_dict[@"shop_goods"];
    GoodDesModel *model1  = [[GoodDesModel alloc]init];
    
    for(NSDictionary *dict in goodsArray)
    {
        GoodDesModel *model = [[GoodDesModel alloc]initWithDict:dict];
        if(model1 ==nil)
        {
            model1 = model;
        }
        ...
        {
            if([model1.goods_express isEqualToString:model.goods_express])
            {
                model1 = model;
            }
            else
