...
- (void)viewDidLoad{
    ...
    if ([self.type isEqualToString:@"0"]) {
         self.expressLabel.text = [self methodJudge:self.method];
    }else{
    
    self.expressLabel.text = @"请选择配送方式";
    }
...
//
   ...
   [footerView.choosePostTypeButton addTarget:self action:@selector(choosePostTypeButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if ([self.type isEqualToString:@"0"]) {
       footerView.postTypeLabel.text =[self methodJudge:self.method];
    }
...
-(NSString *)methodJudge:(NSString *)express
{
    if([express intValue]==0)
    {
        return @"送货上门";
    }
    else if( [express intValue ]==1)
    {
        return @"快递";
    }
    else if ([express intValue]==2)
    {
        return @"自提";
    }
    return nil;
