...
@property (nonatomic,strong)NSMutableArray * stringNameArray;
...
- (void)viewDidLoad
{
    ...
    self.stringNameArray = [NSMutableArray array];
    self.isSearch = NO;
...
- (void)loadCityData:(NSString *)cityID
{
    ...
    [self.stringNameArray removeAllObjects];
...
//                }
                ...
                for (NSDictionary *listDict in arrayCity) {
                    ...
                    for (NSString *city  in listDict[@"list"]) {
                        ...
                        [self.stringNameArray addObject:city];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    if (![textField.text isEqualToString:@""]) {
        @try {
            //调用搜索接口
            [self loadCityData:textField.text];
        }
        @catch (NSException *exception) {
...
- (void)textFieldDidChange:(UITextField *)textField
{
    if ([textField.text isEqualToString:@""]) {
        ...
        NSString * str = [NSString stringWithFormat:@"self CONTAINS '%@' ",textField.text];
        NSPredicate * pre = [NSPredicate predicateWithFormat:str];
NSArray * searchArray = [self.stringNameArray filteredArrayUsingPredicate:pre];
        
        NSMutableArray * array1 = [NSMutableArray array];
        
        for (int i = 0; i < searchArray.count; i ++) {
            CityModel * model = [[CityModel alloc]init];
            model.cityId = searchArray[i];
            model.cityName = searchArray[i];
            [array1 addObject:model];
        }
        
        [self.cityArray removeAllObjects];
        [self.cityArray addObject:array1];
        [self.tableView reloadData];
    }
...
{
    NSArray * arr = [addString componentsSeparatedByString:@"市"];
    _cityLab.text = arr[0];
