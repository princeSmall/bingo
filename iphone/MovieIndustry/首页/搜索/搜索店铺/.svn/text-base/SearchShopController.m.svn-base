...
- (void)viewDidLoad {
    ...
    self.searchTextField.text = self.searchWords;
    self.sort_string = @"";
    self.page = 1;
//    self.authenType = @"";
    ///没有选择是否认证
...
- (void)loadData
{
    ...
    NSMutableDictionary *userDict = [NSMutableDictionary dictionaryWithObjectsAndKeys:APP_DELEGATE.user_id,@"user_id",
                                     ...
                                     self.searchTextField.text,@"shop_name",
                                     [NSString stringWithFormat:@"%ld",self.page],@"page",
                                     self.city_name,@"city_name",
                                     self.category_id,@"category_id", nil];
    
    [HttpRequestServers requestBaseUrl:TIShop_SearchAllShops withParams:userDict withRequestFinishBlock:^(id result)
    {
        ...
        @try {
            if ([dict[@"code"] intValue]==0)
            {
                ...
                else
                {
                    ...
                    {
                        ConllectShopModel *model = [[ConllectShopModel alloc]initWithDict:dic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ConllectShopModel *model = self.shopListArray[indexPath.row];
