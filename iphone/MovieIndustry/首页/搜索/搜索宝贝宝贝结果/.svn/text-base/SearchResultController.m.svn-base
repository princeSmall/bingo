...
- (SelectCertificationView *)certifView
{
    if (!_certifView) {
        ...
        [_certifView.isCertification setTitle:@"器材" forState:UIControlStateNormal];
...
//    NSMutableDictionary *userDict = [NSMutableDictionary dictionaryWithObjectsAndKeys:self.shop_cate_id,@"shop_cate_id",self.saleType,@"sales",self.goodsName,@"goods_name",self.priceType,@"price",[NSString stringWithFormat:@"%ld",self.page],@"p",self.city_id,@"city_id",APP_DELEGATE.user_id,@"user_id" ,nil];
    
    
    /**
     *   sale_count desc(asc)（升序）,goods_price asc(desc)    goods_price asc(desc),sale_count desc(asc)
     */
    ...
    userDict[@"goods_name"] = self.searchTextField.text;
    
    if (!self.shop_cate_id) {
        self.shop_cate_id = @"";
    }
    if (!self.city_id) {
        self.city_id = @"";
    ...
    userDict[@"type"] = self.shop_cate_id;
    userDict[@"people_location"] = self.city_id;
    userDict[@"page"] = [NSString stringWithFormat:@"%ld",self.page];
    
    NSMutableString * mutType = [NSMutableString string];
    switch ([self.saleType intValue]) {
        case 0:[mutType appendString:@""];
            break;
        case 1:[mutType appendString:@"sale_count asc"];
            break;
        case 2:[mutType appendString:@"sale_count desc"];
            break;
        default:
            break;
    }
    
    switch ([self.priceType intValue]) {
        case 0:[mutType appendString:@""];
            break;
        case 1:{
            if (mutType.length > 0) {
                 [mutType appendString:@",goods_price asc"];
            }else{
        ...
        case 2:{
            if (mutType.length > 0) {
                [mutType appendString:@",goods_price desc"];
            }else{
            [mutType appendString:@"goods_price desc"];
            }
            break;
    ...
    userDict[@"sort_string"] = mutType;


    [HttpRequestServers requestBaseUrl:TIGoods_SearchAllGoods withParams:userDict withRequestFinishBlock:^(id result) {
        ...
        @try {
            if ([dict[@"code"] intValue]==0)
            {
                ...
                for (NSDictionary *infoDic in listArray) {
                    BabySearchModel *model = [[BabySearchModel alloc] initWithDict:infoDic];
...
- (void)setNavTabBar
{
    ...
    UIImageView *navImage = [WNController createImageViewWithFrame:CGRectMake(60, 8, 14, 14) ImageName:@"15-07"];
    UITextField *textField = [WNController createTextFieldWithFrame:CGRectMake(54, 2, (isScreen4?132:152), 28) boreStyle:UITextBorderStyleNone font:15];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    goodsDescVc.shopID = model.shop_id;
