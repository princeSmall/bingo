...
@interface SearchIndexController ()<UITextFieldDelegate,UITableViewDataSource,UITableViewDelegate,UIAlertViewDelegate>
...
- (DiscoverPopView *)popView
{
    if (!_popView) {
        ...
        _popView.forumButton.titleLabel.font = KaitiFont;
...
- (void)loadHotWord
{
    ...
    [dict setValue:APP_DELEGATE.user_id forKey:@"user_id"];
    [HttpRequestServers requestBaseUrl:TIGoods_GetSearchHot withParams:dict withRequestFinishBlock:^(id result) {
        NSDictionary *dict = result;
        HHNSLog(@"%@",dict);
        if ([dict[@"code"] intValue]==0) {
            NSArray *listArray = dict[@"data"];
            for (NSDictionary *infoDict in listArray) {
                [self.hotWordArray addObject:infoDict[@"hot_keys"]];
            }
            [_tbView reloadData];
        }
    } withFieldBlock:^{
...
- (void)setNavTabBar
{
    ...
    UITextField *textField = [WNController createTextFieldWithFrame:CGRectMake(74, 2, (isScreen4?132:152), 28) boreStyle:UITextBorderStyleNone font:15];
...
- (void)clearSearchHistory
{
    ...
    UIAlertView * aler = [[UIAlertView alloc]initWithTitle:@"提示" message:@"确认删除搜索记录" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
    aler.delegate = self;
    [aler show];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    }else
    {
        ...
        if ([self.searchType isEqualToString:@"0"]) {//宝贝搜索
        ...
        if ([self.searchType isEqualToString:@"1"]) {//店铺搜索
            if (![textField.text isEqualToString:@""]) {
