...
- (instancetype)cycleScrollViewWithFrame:(CGRect)frame imageURLStringsGroup:(NSArray *)imageURLsGroup AndClickBlock:(ClickImageBlock)block{
    
    if (self == [super initWithFrame:frame]) {
        self.imageURLStringsGroup = [NSMutableArray arrayWithArray:imageURLsGroup];
        self.block = block;
    }
    return self;
}
...
- (instancetype)cycleScrollViewWithFrame:(CGRect)frame imagesGroup:(NSArray *)imagesGroup AndClickBlock:(ClickImageBlock)block{
    if (self == [super initWithFrame:frame]) {
          self.imagesGroup = [NSMutableArray arrayWithArray:imagesGroup];
        self.block = block;
    }
    return self;
}
...
- (void)setupPageControl
{
    ...
    switch (self.pageControlStyle) {
        case SDCycleScrollViewPageContolStyleAnimated:
        {
            ...
            pageControl.dotSize = CGSizeMake(10, 10);
...
//         cell.imageView.contentMode =  UIViewContentModeScaleAspectFill;
    ...
    cell.imageBtn.tag = 333 + itemIndex;
    [cell.imageBtn addTarget:self action:@selector(printfNum:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)printfNum:(UIButton *)sender{
