...
- (UICollectionView *)collectView
{
    if (!_collectView) {
        ...
        [_collectView registerNib:[UINib nibWithNibName:@"IndexGoodsCollectionCell1" bundle:nil] forCellWithReuseIdentifier:@"IndexGoodsCollectionCellID"];
...
#warning 这边是城市选择（左），闪电租（右）按钮
    ...
    UILabel *navLabel = [WNController createLabelWithFrame:CGRectMake(0, 1, 30, 30) Font:14 Text:@"上海" textAligment:NSTextAlignmentLeft];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    IndexGoodsCollectionCell1 * cell = [collectionView dequeueReusableCellWithReuseIdentifier:CellIdentifier forIndexPath:indexPath];
    
    if (indexPath.section == 2) {
        cell.send.hidden = YES;
        cell.place.hidden = YES;
        cell.yajin.hidden = YES;
    }else{
        cell.yajin.hidden = NO;
        cell.send.hidden = NO;
        cell.place.hidden = NO;
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(kind == UICollectionElementKindSectionHeader)
    {
        ...
        headerView.button.imageView.contentMode =  UIViewContentModeScaleAspectFill;
...
- (void)cityName:(NSString *)CityName andCityId:(NSString *)cityId
{
    ...
    CGSize size = [CityName boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:dic context:nil].size;
    _locationLabel.frame =CGRectMake(0, 1, size.width, 30);

    self.navImage.frame =CGRectMake(size.width, 8, 18, 18);
