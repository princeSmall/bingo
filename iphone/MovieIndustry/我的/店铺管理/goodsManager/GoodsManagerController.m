...
- (void)viewDidLoad{
    ...
    [self setNavTabBar:@"商品管理"];
      [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(ReloadDataNewsss) name:@"修改信息成功" object:nil];
...
{
    self.mainTableView = [[UITableView alloc] initWithFrame:CGRectMake(0,CGRectGetMaxY(self.segmentView.frame)+1, kViewWidth,kViewHeight-CGRectGetMaxY(self.segmentView.frame)-65) style:UITableViewStyleGrouped];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CartGood * good = _allGoodsArray[a];
        cell.goodsName.text = good.goods_name;
#warning 缺少送货方式
#warning 缺少送货方式
#warning 缺少送货方式字段
       // cell.deliveryWay
        cell.price.text = [NSString stringWithFormat:@"￥%d",[good.goods_price intValue]];
        cell.yajinLabel.text = [NSString stringWithFormat:@"￥%d",[good.goods_deposit intValue]];
        ...
        cell.address.text = good.people_location;
        cell.rightBtn.hidden = NO;
        cell.xiugaiBtn.tag = 666 + indexPath.row;
    
        [cell.xiugaiBtn addTarget:self action:@selector(ChangeGoodsDes:) forControlEvents:UIControlEventTouchUpInside];
    }
    else{
        if (1 == _chooseIndex) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (self.chooseIndex) {
        case 0:{
            ...
            if ([goodsModel.type isEqual:@"0"]) {
                if ([goodsModel.goods_express isEqual:@"0"]) {
                    goodsInfo.type = @"送货上门";
                }
                if ([goodsModel.goods_express isEqual:@"1"]) {
                    goodsInfo.type = @"快递";
                }
                if ([goodsModel.goods_express isEqual:@"2"]) {
                   goodsInfo.type = @"买家自提";
                }
            }
