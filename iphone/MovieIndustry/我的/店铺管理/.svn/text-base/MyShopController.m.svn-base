...
//    [myView1 addSubview:smImageView];
    ...
    CGFloat nameW = screenWidth-160-25;
    float n = 1;
    CGFloat nameH = ceilf(n) * size.height;
    CGFloat nameW1 = [DeliveryUtility caculateContentSizeWithContent:shopName andHight:21.0f andWidth:MAXFLOAT andFont:DefaultFont].width;
    ...
    //店铺等级
    IconStorlView * starView1 = [[IconStorlView alloc]initWithFrame:CGRectMake(self.shopNameLabel.frame.origin.x, 20 + self.shopNameLabel.frame.size.height, 88, 16) AndScore:[self.storeModel.sale_count intValue]];
  
    [myView1 addSubview:starView1];
    
    //打分星星
     StarView * starView = [[StarView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(starView1.frame)+10, 20 + self.shopNameLabel.frame.size.height, 88, 16) score:[self.storeModel.score intValue] canscore:@"0"];
...
//    if (nameW1 < screenWidth - 160 - 25) {
//        Sx =nameW1 +5 + 160;
//    }else{
//        Sx = screenWidth -30;
//    }

    Sx = self.shopNameLabel.frame.origin.x;
    UIImageView *levelImage = [WNController createImageViewWithFrame:CGRectMake(Sx,CGRectGetMaxY(self.shopNameLabel.frame)-5, 30, 30) ImageName:@"shop_03.png"];
//    [myView1 addSubview:levelImage];
    ...
    CGRect brifyFrame = CGRectMake(CGRectGetMaxX(_shopImageView.frame)+10, 55, brifyW, 90);
