...
@property (nonatomic,strong)UILabel * goods_job;//职称；
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.row == 10){
        ...
        if (self.desModel) {
            cell.myLabel.text = self.desModel.category_name;
        }
    ...
    if (indexPath.row == 12) {
        TTIInputCell * cell = [[TTIInputCell alloc]initWithTableView:tableView];
        [cell inputWithSring:@"特点"];
        cell.inputTxfield.delegate = self;
        if (self.desModel) {
            cell.inputTxfield.text = self.desModel.goods_alone;
        }
        ...
        if (self.desModel) {
            ...
            cell.textView.text = self.desModel.goods_desc;
        }
 ...
 */
    ...
    if ([self.goods_category_id.text isEqual:@"请选择类型"]) {
