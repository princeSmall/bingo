...
-(void)loadData
{
    NSMutableDictionary *userDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:APP_DELEGATE.user_id,@"user_id",self.type,@"category_id",@"2",@"cat_type", nil];
...
{
    PublishThirdCategoryController *thirdController = [[PublishThirdCategoryController alloc]init];
    thirdController.category_id =[self.categoryArray objectAtIndex:indexPath.row][@"category_id"];
    thirdController.category_name = [self.categoryArray objectAtIndex:indexPath.row][@"category_name"];
    thirdController.thirdBackFn = ^(NSDictionary *dict){
        self.backFn (dict);
    };
