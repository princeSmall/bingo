...
@property (strong, nonatomic) IBOutlet UITextField *makPrice;//市场价
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(publishSiteTextViewEditChanged:) name:@"UITextViewTextDidChangeNotification" object:self.makPrice];
...
#warning 选择类型没有做TUPIAN
    ...
    self.yajinSwitch.on = NO;
    self.yajinSwitch.userInteractionEnabled = NO;
    [self yajinSitch:nil];
}
- (void)initIssueSiteView
{
    ...
    ladelNotice.text = @"设置商品详情，请联系平台在后台设置";
...
- (void)addInputeWordNotification
{
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(publishSiteTextFiledEditChanged:) name:@"UITextFieldTextDidChangeNotification" object:self.txtSiteArea];
          [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(publishSiteTextFiledEditChanged:) name:@"UITextFieldTextDidChangeNotification" object:self.makPrice];
...
//    self.makPrice.text = self.desModel.market_price;
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(publishSiteTextViewEditChanged:) name:@"UITextViewTextDidChangeNotification" object:self.textViewDetail];
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(publishSiteTextViewEditChanged:) name:@"UITextViewTextDidChangeNotification" object:self.makPrice];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(publishSiteTextViewEditChanged:) name:@"UITextViewTextDidChangeNotification" object:self.txtPrice];
}
...
- (void)publishSiteTextFiledEditChanged:(NSNotification *)noti
{
    ...
    else if(textField == self.makPrice){
        kMaxLength = 6;
    }else{
        kMaxLength = 10;
    }
    ...
    if ([self checkPublishSiteInfoValid]) {
        ...
        [HttpRequestServers requestBaseUrl:TIPublish_UpdatePlace withParams:self.siteDict withRequestFinishBlock:^(id result) {
            if ([result[@"code"] intValue] == 0) {
                HUD.labelText = @"修改成功";
                [HUD hide:YES];
                  [[NSNotificationCenter defaultCenter]postNotificationName:@"商品修改成功" object:nil];
...
- (BOOL)checkPublishSiteInfoValid
{
    ...
    self.txtSiteName.delegate = self;
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    ...
    if ([textField isEqual:self.makPrice]) {
