...
//    [self CreateBalanceView];
    ...
    //发送网络请求
    [self getMineInfoRequest];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(getMineInfoRequestSSS) name:@"storeOpenSuccess" object:nil];
    // Do any additional setup after loading the view from its nib.
}

- (void)getMineInfoRequestSSS{
    [self getMineInfoRequest];
}
...
- (void)refreshMineInfomations{
  ...
  [self.headImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",TIMIDDLEImage,self.desModel.img]] placeholderImage:[UIImage imageNamed:@"defualt_headerImg"]];
    ...
    if (data) {
        ...
        self.headBackGroundView.image = [[UIImage imageNamed:@"testImage.jpg"] blurredImage:0.5];
    }
...
- (void)CreateMainTableView{
    ...
    self.mainTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.headView.frame), self.view.frame.size.width, self.view.frame.size.height-CGRectGetMaxY(self.headView.frame)+115)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 2) {
        [cell SetTitle:@"收藏" AndIcon:[UIImage imageNamed:@"site4.png"]];
    }
    if (indexPath.row == 3) {
...
//    self.nameString = @"诸葛亮诸葛亮";
    self.desString = self.desModel.job;
    self.fansString = @"0";
    self.noticeString = @"0";
    self.countString = @"0";
    //姓名
    ...
    if ([self.desModel.job isEqualToString:@""]) {
        [self.labelLine removeFromSuperview];
        
    }else{
        UILabel * lineLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.labelName.frame)+10, self.labelName.frame.origin.y, 0.7, self.labelName.frame.size.height)];
    ...
    UILabel * labelZC = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.labelLine.frame) +10, CGRectGetMaxY(self.labelName.frame)-ZCSize.height, [UIScreen mainScreen].bounds.size.width - CGRectGetMaxX(self.labelLine.frame)-30, ZCSize.height)];
...
//        [self.navigationController pushViewController:photosVc animated:YES];
        ...
        if([self.desModel.has_shop isEqualToString:@"0"]){
            ...
            [createShopVC setHidesBottomBarWhenPushed:YES];
